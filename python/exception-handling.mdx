---
title: "Python Exception Handling: try, except, else, finally, and Custom Exceptions"
sidebarTitle: "Exceptions"
description: "Handle errors in Python using try-except blocks, process specific exception types, use else and finally clauses, and create custom exception classes — from Clase 1."
---
...
<Note>
Catching bare `Exception` as a permanent solution is an anti-pattern. It silences errors you did not anticipate (such as `KeyboardInterrupt` bubbling up from nested code) and makes debugging much harder. Treat it as a last-resort catch-all, not a first line of defence.
</Note>
...
```python raising the custom exception
from NumerosIgualesException import NumerosIgualesException
