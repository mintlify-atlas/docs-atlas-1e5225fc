---
title: "N-Queens Solver: Backtracking Algorithm in Vanilla JavaScript"
sidebarTitle: "N-Queens"
description: "Explore a browser-based N-Queens puzzle solver built with vanilla JavaScript using recursive backtracking — place N queens on an N×N board with no conflicts, then render solutions as visual grids."
---
...
The solver lives inside the `resolver()` function, which is triggered when the user clicks the button. It initialises an N×N board filled with `'.'` characters, then calls the recursive `backtrack()` function starting from row 0.

<CodeGroup>

```javascript title="resolver() — entry point"
function resolver() {
  ...
  if (isNaN(n) || n < 4 || n > 13) {
    contenedor.innerHTML =
      "<p style='color: #dc2626; ...'>⚠️ Error: Por favor, introduce un número entre 4 y 13.</p>";
    return;
  }
...
```javascript title="backtrack(fila) — recursive solver"
function backtrack(fila) {
  // Base case: all N rows filled → valid solution found
  if (fila === n) {
