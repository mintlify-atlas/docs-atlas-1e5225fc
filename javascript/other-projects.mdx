---
title: "Rock-Paper-Scissors, Knight's Tour, Avatar Game, and Login UI Projects"
sidebarTitle: "Other Projects"
description: "Four additional browser JavaScript projects from the course: a Rock-Paper-Scissors game, Knight's Tour puzzle solver, an Avatar character game, and an animated login form with CSS effects."
---
...
The solution is printed to the console as a numbered grid showing the visit order of each cell. Execution time is measured with `Date.now()` before and after the call.
...
The player picks a character via radio buttons and clicks *Seleccionar*. The `seleccionarPersonajeJugador()` function reads which radio is `checked`, updates the display, and immediately triggers `seleccionarPersonajeEnemigo()` to assign a random opponent:
...
```html title="index.html — animated background structure (simplified)"
<section>
  <div class="color"></div>
  <div class="color"></div>
  <div class="color"></div>
